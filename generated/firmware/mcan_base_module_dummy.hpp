...
#include "mcan_base_module_dummy_types.hpp"
...
struct Hardware_t {
  ...
  static constexpr const char *k_description =
      "Base module required for all modules";
};
 ...
 * @details This will trigger all devices on the bus to respond with their uniq
 * ID that defines the hardware
 * @permission r
 */
...
/**
 * @brief Ping Module
 * @details Ping the module to check if it's alive, the counter will increment
 * on each ping
 * @default 0
 * @permission w
 */
 ...
 * @details Command to flash the indicator LED on the module, useful for
...
public:
  ...
  auto get_write_callbacks() {
    return std::make_tuple(
        std::make_pair(&McCanSlaveInterface_t::callback_write_dummy_command,
                       &McCanSlaveInterface_t::dummy_command));
  }

  auto get_read_variables() {
    return std::make_tuple(&McCanSlaveInterface_t::dummy_command);
  }
